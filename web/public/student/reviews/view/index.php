...
if($_SERVER['REQUEST_METHOD']==='POST'){try{$submissionId=learning_submit_review($id,(int)$user['id'],$_POST);engagement_log_activity((int)$user['id'],'review_taken','review_test',(string)$id,['submission_id'=>$submissionId]);header('Location: /student/reviews/result?id='.$id);exit;}catch(Throwable $e){$error=$e->getMessage();}}

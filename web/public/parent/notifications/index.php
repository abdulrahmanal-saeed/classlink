 ...
 * Actionable parent notifications. Own parent account only.
 */
...
$notifications = comm_user_notifications((int) $user['id'], 'parent');

ob_start();
?>
<p class="text-muted">Parent notifications and child learning updates. Action buttons respect linked-child routes and mark notifications as read.</p>
    ...
    <?php foreach ($notifications as $notification): ?>
      <div class="list-group-item px-0">
        ...
        <?php if (!empty($notification['action_label']) && !empty($notification['action_url'])): ?>
          <a class="btn btn-sm btn-outline-brand mt-2" href="/parent/notifications?read=<?= (int) $notification['id'] ?>&to=<?= urlencode($notification['action_url']) ?>"><?= htmlspecialchars($notification['action_label'], ENT_QUOTES, 'UTF-8') ?></a>
        <?php endif; ?>
      </div>
    <?php endforeach; ?>
  </div>

...
/**
 * /parent/referrals
 * Parent referral link and reward tracking.
 */
...
<div class="foundation-card mb-4"><h2 class="h5 fw-bold">Terms</h2><p class="mb-0"><?= nl2br(htmlspecialchars($terms, ENT_QUOTES, 'UTF-8')) ?></p></div>
<div class="foundation-card">
  <h2 class="h5 fw-bold">My referrals</h2>
  <?php if (!$referrals): ?><div class="alert alert-light border mb-0">No referrals yet.</div><?php else: ?>
    <div class="table-responsive"><table class="table table-hover align-middle"><thead><tr><th>Date</th><th>Referred</th><th>Status</th><th>Reward</th></tr></thead><tbody>
    <?php foreach ($referrals as $referral): ?><tr><td><?= htmlspecialchars($referral['created_at'], ENT_QUOTES, 'UTF-8') ?></td><td><?= htmlspecialchars($referral['referred_user_name'] ?? $referral['referred_name'] ?? $referral['referred_email'] ?? '-', ENT_QUOTES, 'UTF-8') ?></td><td><span class="badge text-bg-light border"><?= htmlspecialchars($referral['status'], ENT_QUOTES, 'UTF-8') ?></span></td><td><?= htmlspecialchars(($referral['reward_type'] ?? '-') . ' ' . ($referral['reward_value'] ?? ''), ENT_QUOTES, 'UTF-8') ?></td></tr><?php endforeach; ?>
    </tbody></table></div>
  <?php endif; ?>
</div>
<?php
$content = ob_get_clean();
render_dashboard_shell($user, 'Parent Referrals', $content);
